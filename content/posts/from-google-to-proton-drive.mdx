...
First that came to my mind was a good old <code>cp -r</code>. This eventualy
...
similar to <code>rclone</code>. However, it's mostly focused at synchronising
...
After the transfer is complete, it's important to validate
the transfer was successful. We can check the logs
and use [rclone check](https://rclone.org/commands/rclone_check/).

The <code>rclone check</code> command is explicitly made for this task,
as the docs say it: "checks the files in the source and destination match."
